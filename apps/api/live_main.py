...
from .main import app, runtime, _learning_store
...
def list_signals():
    ...
    endpoint therefore exposes real evidence as live attention signals instead
    of returning cockpit fixture data.
    """
    items: list[dict[str, Any]] = []
    for evidence in runtime.store.evidence.values():
        reliability = float(getattr(evidence, "reliability", 0.5) or 0.5)
        signal_id = _stable_uuid("signal", str(evidence.id))
        items.append(
            {
                "id": signal_id,
                ...
                "novelty": reliability,
                "urgency": float(getattr(evidence, "metadata", {}).get("urgency", 0.0) or 0.0),
                "commercial_upside": float(
                    getattr(evidence, "metadata", {}).get("commercial_upside", 0.0) or 0.0
                ),
...
def list_curiosity_tasks():
    ...
    for belief in runtime.store.beliefs.values():
        for index, unknown in enumerate(getattr(belief, "unknowns", []) or []):
            task_id = _stable_uuid("curiosity", f"{belief.id}:{index}:{unknown}")
            items.append(
                {
                    "id": task_id,
                    "created_at": _iso(getattr(belief, "updated_at", None)),
