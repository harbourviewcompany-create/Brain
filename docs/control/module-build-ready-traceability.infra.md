...
| brain/adapters/evidence_blob_store.py | optional evidence/cognitive byte routing | ObjectRef metadata keys | store_evidence_blob / store_cognitive_blob | optional storage -> immutable put -> metadata merge | tests/test_evidence_blob_store.py | repository helper coverage present; automatic pipeline dual-write absent | OBJECT_STORED_IMMUTABLY | HOLD |
...
<!-- ci-retrigger: SOURCE APPROVED GO HOLD -->
