...
class ContinuousCognitionRunner:
    ...
    def _run_endogenous(self) -> bool:
        ...
        thought = self.mind.next_endogenous_stimulus(
            beliefs=beliefs,
            working_memory_items=wm_items or None,
            self_status=status,
        ...
        try:
            result = self.cycle.process(
                CognitiveStimulus(
                    ...
                    source_id=thought.source_key,
                    claim=thought.claim,
                    source_reliability=thought.source_reliability,
                    supports=True,
                    belief_statement=thought.claim,
                    ...
                    uncertainty_reduction=thought.uncertainty_reduction,
                    noise_probability=0.05,
                    operator_burden=0.0,
                    metadata=thought.as_inbox_payload().get("metadata") or {},
                )
            )
    ...
    def _after_cycle(self, result: Any, *, claim: str, content: str, source_key: str) -> None:
        ...
        try:
            self.mind.refresh_policy(
                ...
                open_curiosity=self.mind.curiosity_open_count(),
        ...
        try:
            pred = Prediction(
                ...
                metadata={
                    "endogenous": True,
                    "claim": statement[:200],
                    "cycle_id": str(getattr(result, "id", "")),
                    "source_key": source_key,
                },
            )
            if hasattr(self.learning, "create_prediction"):
                self.learning.create_prediction(pred)
        ...
        try:
            if hasattr(self.learning, "record_outcome"):
                result = self.learning.record_outcome(
                    ...
                    edge_ids=list(edge_ids or []) or None,
                    source_keys=list(source_keys or [ENDOGENOUS_SOURCE_KEY]),
                )
    ...
    def _enqueue_thought(self, thought: EndogenousStimulus) -> Any:
        if not hasattr(self.inbox, "enqueue"):
            return None
        try:
