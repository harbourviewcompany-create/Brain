...
class Mood:
    ...
    def integrate(self, emotion: EmotionalState) -> None:
        alpha = (1.0 - self.inertia) * emotion.intensity
        self.valence = max(-1.0, min(1.0, (1 - alpha) * self.valence + alpha * emotion.valence))
        self.arousal_baseline = max(
            0.0, min(1.0, (1 - alpha) * self.arousal_baseline + alpha * emotion.arousal)
        )

    def appraisal_bias(self) -> float:
        return 0.15 * self.valence


def _classify(appraisal: AppraisalInput, valence: float, arousal: float) -> DiscreteEmotion:
    if appraisal.novelty > 0.75 and appraisal.certainty < 0.4:
        return DiscreteEmotion.SURPRISE
    if appraisal.norm_compatibility < -0.6:
        return DiscreteEmotion.DISGUST
    if valence >= 0.25:
        if appraisal.agency == "self" and appraisal.controllability >= 0.5:
            return DiscreteEmotion.PRIDE
        if appraisal.urgency < 0.3 and appraisal.novelty < 0.4:
            return DiscreteEmotion.RELIEF
        if appraisal.novelty >= 0.45:
    ...
    if valence <= -0.25:
        ...
        if appraisal.agency == "other" and appraisal.controllability >= 0.3:
