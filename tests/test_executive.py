...
def test_no_conflict_when_prepotent_and_goal_best_agree() -> None:
    good_habit = ResponseCandidate(
        action="do_the_right_thing",
        source=ResponseSource.HABITUAL,
        prepotency=0.9,
        goal_alignment=0.9,
        expected_value=0.8,
    )
...
def test_full_control_resource_overrides_prepotent_impulse() -> None:
    ...
    decision = svc.arbitrate(
        ...
        goals=None,
        control=control,
        modulation=_neutral_modulation(),
    )
    assert decision.override_attempted
    assert decision.override_succeeded
...
def test_depleted_control_resource_falls_back_to_impulse() -> None:
    ...
    assert decision.override_attempted
    assert not decision.override_succeeded
...
def test_high_stress_reduces_effective_control_and_raises_override_cost() -> None:
    ...
    svc = ExecutiveControlService()
    calm_decision = svc.arbitrate(
        [impulsive, deliberate], goals=None,
        control=CognitiveControlResource(capacity=1.0, current=0.5), modulation=calm,
    )
    stressed_decision = svc.arbitrate(
        [impulsive, deliberate], goals=None,
        control=CognitiveControlResource(capacity=1.0, current=0.5), modulation=stressed,
    )
    assert stressed_decision.effective_control < calm_decision.effective_control
    assert stressed_decision.control_cost > calm_decision.control_cost
...
def test_goal_deficit_increases_pressure_to_override() -> None:
    ...
    urgent_goal = [CognitiveDrive(name="deadline", target=1.0, current=0.0, priority=1.0)]
    idle_goal = [CognitiveDrive(name="deadline", target=1.0, current=1.0, priority=1.0)]

    svc = ExecutiveControlService()
    urgent_decision = svc.arbitrate(
        [impulsive, deliberate], goals=urgent_goal,
        control=CognitiveControlResource(capacity=1.0, current=1.0), modulation=_neutral_modulation(),
    )
    idle_decision = svc.arbitrate(
        [impulsive, deliberate], goals=idle_goal,
        control=CognitiveControlResource(capacity=1.0, current=1.0), modulation=_neutral_modulation(),
    )
    assert urgent_decision.control_cost > idle_decision.control_cost
