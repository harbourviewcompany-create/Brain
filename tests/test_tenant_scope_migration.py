...
def test_migration_adds_tenant_id_to_existing_cognitive_tables():
    sql = _sql()
    required_tables = [
        "brain_events", "sources", "observations", "evidence", "entities", "beliefs",
        "belief_evidence", "graph_nodes", "graph_edges", "rewire_events", "actions", "outcomes",
        "memory_items", "bitemporal_facts", "neuromodulator_snapshots", "homeostatic_snapshots",
        "cognitive_tasks", "cognitive_experiments", "cognitive_experiment_results", "projection_checkpoints",
        "sensory_inbox", "cognitive_cycle_runs", "predictions", "attribution_records", "working_memory_snapshots",
        "revenue_signals", "scored_revenue_opportunities", "packaged_offers", "revenue_experiments",
        "revenue_experiment_results", "daily_revenue_reports", "economic_objects", "economic_transitions",
...
def test_migration_creates_baseline_tenant_policies_and_forces_rls():
    sql = _sql()
    for marker in ["tenant_isolation_select", "tenant_isolation_insert", "tenant_isolation_update", "tenant_isolation_delete"]:
...
def test_migration_makes_safe_natural_uniqueness_tenant_scoped():
    sql = _sql()
    for marker in ["sources_tenant_key_unique_idx", "entities_tenant_kind_key_unique_idx", "graph_nodes_tenant_kind_key_unique_idx", "daily_revenue_reports_tenant_date_unique_idx"]:
        assert marker in sql
